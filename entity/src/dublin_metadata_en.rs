//! `SeaORM` Entity, @generated by sea-orm-codegen 1.1.20

use sea_orm::entity::prelude::*;

#[derive(Clone, Debug, PartialEq, DeriveEntityModel, Eq)]
#[sea_orm(table_name = "dublin_metadata_en")]
pub struct Model {
    #[sea_orm(primary_key)]
    pub id: i32,
    pub title: String,
    pub description: Option<String>,
    pub location_en_id: Option<i32>,
    pub creator_en_id: Option<i32>,
}

#[derive(Copy, Clone, Debug, EnumIter, DeriveRelation)]
pub enum Relation {
    #[sea_orm(has_many = "super::accession::Entity")]
    Accession,
    #[sea_orm(
        belongs_to = "super::dublin_metadata_creator_en::Entity",
        from = "Column::CreatorEnId",
        to = "super::dublin_metadata_creator_en::Column::Id",
        on_update = "Cascade",
        on_delete = "SetNull"
    )]
    DublinMetadataCreatorEn,
    #[sea_orm(has_many = "super::dublin_metadata_en_contributors::Entity")]
    DublinMetadataEnContributors,
    #[sea_orm(has_many = "super::dublin_metadata_en_relations::Entity")]
    DublinMetadataEnRelations,
    #[sea_orm(has_many = "super::dublin_metadata_en_subjects::Entity")]
    DublinMetadataEnSubjects,
    #[sea_orm(
        belongs_to = "super::dublin_metadata_location_en::Entity",
        from = "Column::LocationEnId",
        to = "super::dublin_metadata_location_en::Column::Id",
        on_update = "Cascade",
        on_delete = "SetNull"
    )]
    DublinMetadataLocationEn,
}

impl Related<super::accession::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::Accession.def()
    }
}

impl Related<super::dublin_metadata_creator_en::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::DublinMetadataCreatorEn.def()
    }
}

impl Related<super::dublin_metadata_en_contributors::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::DublinMetadataEnContributors.def()
    }
}

impl Related<super::dublin_metadata_en_relations::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::DublinMetadataEnRelations.def()
    }
}

impl Related<super::dublin_metadata_en_subjects::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::DublinMetadataEnSubjects.def()
    }
}

impl Related<super::dublin_metadata_location_en::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::DublinMetadataLocationEn.def()
    }
}

impl Related<super::dublin_metadata_relation_en::Entity> for Entity {
    fn to() -> RelationDef {
        super::dublin_metadata_en_relations::Relation::DublinMetadataRelationEn.def()
    }
    fn via() -> Option<RelationDef> {
        Some(
            super::dublin_metadata_en_relations::Relation::DublinMetadataEn
                .def()
                .rev(),
        )
    }
}

impl Related<super::dublin_metadata_subject_en::Entity> for Entity {
    fn to() -> RelationDef {
        super::dublin_metadata_en_subjects::Relation::DublinMetadataSubjectEn.def()
    }
    fn via() -> Option<RelationDef> {
        Some(
            super::dublin_metadata_en_subjects::Relation::DublinMetadataEn
                .def()
                .rev(),
        )
    }
}

impl ActiveModelBehavior for ActiveModel {}
