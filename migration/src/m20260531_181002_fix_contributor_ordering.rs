...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_index(
                Index::create()
                    .name("idx_dublin_metadata_en_contributors_order_id")
                    .table(DublinMetadataEnContributors::Table)
                    .col(DublinMetadataEnContributors::OrderId)
                    .to_owned(),
            )
            .await?;
        ...
        manager
            .create_index(
                Index::create()
                    .name("idx_dublin_metadata_ar_contributors_order_id")
                    .table(DublinMetadataArContributors::Table)
                    .col(DublinMetadataArContributors::OrderId)
                    .to_owned(),
            )
            .await?;
        ...
        manager
            ...
            .execute_unprepared(
                ...
                SELECT
                    ...
                    (
                    SELECT array_agg(dmoec.contributor_id ORDER BY dmoec.order_id)
                    FROM dublin_metadata_en_contributors dmoec
                    WHERE dmoec.metadata_id = a.dublin_metadata_en
                    LIMIT 200
                    ) AS contributor_en_ids,
                    (
                    SELECT array_agg(COALESCE(dmcre.role, '') ORDER BY dmoec.order_id)
                    ...
                    (
                    SELECT array_agg(COALESCE(dmoec.role_id, 0) ORDER BY dmoec.order_id)
                    FROM dublin_metadata_en_contributors dmoec
                    WHERE dmoec.metadata_id = a.dublin_metadata_en
                    LIMIT 200
                    ...
                    (
                    SELECT array_agg(dmoac.contributor_id ORDER BY dmoac.order_id)
                    FROM dublin_metadata_ar_contributors dmoac
                    WHERE dmoac.metadata_id = a.dublin_metadata_ar
                    LIMIT 200
                    ) AS contributor_ar_ids,
                    (
                    SELECT array_agg(COALESCE(dmcra.role, '') ORDER BY dmoac.order_id)
                    ...
                    (
                    SELECT array_agg(COALESCE(dmcra.id, 0) ORDER BY dmoac.order_id)
                    FROM dublin_metadata_contributor_role_ar dmcra
                    LEFT JOIN dublin_metadata_ar_contributors dmoac ON dmcra.id = dmoac.role_id
                    LEFT JOIN dublin_metadata_ar dma3 ON dma3.id = dmoac.metadata_id
                    WHERE dma3.id = a.dublin_metadata_ar
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
            ...
            .execute_unprepared(
               ...
               CREATE OR REPLACE VIEW accessions_with_metadata AS
                SELECT
                    ...
                    (
                    SELECT array_agg(COALESCE(dmoec.role_id, 0))
                    FROM dublin_metadata_en_contributors dmoec
                    WHERE dmoec.metadata_id = a.dublin_metadata_en
                    LIMIT 200
                    ) AS contributor_role_en_ids,    
                    ...
                    (
                    SELECT array_agg(COALESCE(dmcra.id, 0))
                    FROM dublin_metadata_contributor_role_ar dmcra
                    LEFT JOIN dublin_metadata_ar_contributors dmoac ON dmcra.id = dmoac.role_id
                    LEFT JOIN dublin_metadata_ar dma3 ON dma3.id = dmoac.metadata_id
                    WHERE dma3.id = a.dublin_metadata_ar
        ...
        manager
            .get_connection()
            .execute_unprepared("ALTER TABLE dublin_metadata_en_contributors DROP COLUMN order_id")
            .await?;

        manager
            .get_connection()
            .execute_unprepared("ALTER TABLE dublin_metadata_ar_contributors DROP COLUMN order_id")
            .await?;
